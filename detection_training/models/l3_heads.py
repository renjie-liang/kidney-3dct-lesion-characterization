...
class L3Output:
    """Container for L3 predictions (mass/tumor merged into `solid`)."""

    def __init__(self, exists, cyst, solid, size, enhancement, attenuation):
        self.exists = exists              # (B, 6)
        self.cyst = cyst                  # (B, 6)
        self.solid = solid                # (B, 6)
        self.size = size                  # (B, 6)
...
class L3HeadA(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim=256):
        ...
        # Per side: count(4) + cyst + solid + size + enhancement(2) + attenuation(4) = 12
        self.side_head = nn.Linear(hidden_dim, 12)
    ...
    def _to_slot_format(self, left, right):
        ...
        for side_out in [left, right]:
            ...
            solid = side_out[:, 5:6]
            size = side_out[:, 6:7]
            enh = side_out[:, 7:9]           # (B, 2)
            att = side_out[:, 9:12]          # (B, 3), pad to 4
        ...
        return L3Output(
            ...
            solid=torch.cat(all_solid, dim=1),
...
class L3HeadB(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim=256):
        ...
        # Per slot: exists + cyst + solid + size + enhancement(2) + attenuation(4) = 10
...
class L3HeadC(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim=256):
        ...
        # Per slot: exists + cyst + solid + size + enhancement(2) + attenuation(4) = 10
        self.slot_out = nn.Linear(hidden_dim, 10)
    ...
    def forward(self, features):
        ...
        out = torch.stack(slots, dim=1)  # (B, 6, 10)
...
class L3HeadD(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim=256):
        super().__init__()
        # 6 slots × 10 features (exists + cyst + solid + size + enh(2) + att(4))
        self.head = nn.Sequential(
            nn.Linear(input_dim, hidden_dim), nn.ReLU(), nn.Dropout(0.3),
            nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Dropout(0.3),
            nn.Linear(hidden_dim, TOTAL_SLOTS * 10),
        )

    def forward(self, features):
        out = self.head(features).view(-1, TOTAL_SLOTS, 10)  # (B, 6, 10)
...
def compute_l3_loss(output: L3Output, extra: dict, labels: dict, approach: str,
    ...
    gt_solid = torch.stack([labels["l3_left_solid"], labels["l3_right_solid"]], dim=1).view(B, TOTAL_SLOTS)
    gt_solid_v = torch.stack([labels["l3_left_solid_valid"], labels["l3_right_solid_valid"]], dim=1).view(B, TOTAL_SLOTS)
    ...
    if approach == "B":
        ...
        gt_solid = _apply_perm(gt_solid, perm)
        gt_solid_v = _apply_perm(gt_solid_v, perm)
    ...
    for name, pred, gt, valid in [
        ...
        ("solid", output.solid, gt_solid, gt_solid_v),
    ]:
    ...
    if hier_mode != "l1":
        for feat_name, feat_logits, gt_label_key in [
            ...
            ("solid", output.solid, "has_solid"),
        ]:
            slot_probs = torch.sigmoid(feat_logits)  # (B, 6)
            left_prob = 1 - (1 - slot_probs[:, :3]).prod(dim=1)
            right_prob = 1 - (1 - slot_probs[:, 3:]).prod(dim=1)
