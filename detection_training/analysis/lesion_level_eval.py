...
def evaluate_method_D(predictions, size_tol=1.0, matching="coco"):
    ...
    Classes: cyst, solid. mAP = unweighted mean of AP_cyst, AP_solid.
    Classes with no positives (AP=NaN) are excluded from mAP, per COCO convention.
    """
    ...
    valid_aps = [per_class[c]["AP"] for c in ["cyst", "solid"]
...
def evaluate_method_D_coco(predictions, size_tol=1.0):
    ...
    mAP = float(np.mean([per_class[c]["AP"] for c in ["cyst", "solid"]
                         if not np.isnan(per_class[c]["AP"])]))
    return {
        "method": "D_coco",
...
def evaluate_method_A(predictions, conf_thresh=0.5, size_tol=1.0, weights=None):
    ...
    sz_p = predictions["size_pred"]
    sz_g = predictions["size_gt"]
    pred_type = {k: predictions[f"{k}_pred"] for k in ["cyst", "solid"]}
    gt_type   = {k: predictions[f"{k}_gt"]    for k in ["cyst", "solid"]}
    gt_valid  = {k: predictions[f"{k}_valid"] for k in ["cyst", "solid"]}
    N = ex_p.shape[0]

    tp = fp = fn = 0
    # Per-attribute accuracy on TP
    attr_totals = {a: {"correct": 0, "total": 0} for a in ["cyst", "solid"]}
    size_errors = []
    ...
    for n in range(N):
        for sl, side_name in [(slice(0, 3), "L"), (slice(3, 6), "R")]:
            p_t = {k: pred_type[k][n, sl] for k in ["cyst", "solid"]}
            g_t = {k: gt_type[k][n, sl]   for k in ["cyst", "solid"]}
            g_v = {k: gt_valid[k][n, sl]  for k in ["cyst", "solid"]}
    ...
    return {
        ...
        "solid_acc_on_tp": (attr_totals["solid"]["correct"] / attr_totals["solid"]["total"]
                            if attr_totals["solid"]["total"] > 0 else None),
...
WEIGHT_PRESETS = {
    ...
    "size_only":      {"w_size": 1.0, "w_cyst": 0.0, "w_solid": 0.0, "size_scale": 10.0},
    # Balanced: size + type equally
    "balanced":       {"w_size": 1.0, "w_cyst": 1.0, "w_solid": 1.0, "size_scale": 10.0},
    # Type-heavy: emphasize type accuracy
    "type_heavy":     {"w_size": 0.5, "w_cyst": 2.0, "w_solid": 2.0, "size_scale": 10.0},
    # Size-heavy: emphasize size accuracy (radiologic: measurement accuracy)
    "size_heavy":     {"w_size": 3.0, "w_cyst": 0.5, "w_solid": 1.0, "size_scale": 10.0},
}
...
def main():
    ...
    OUT_DIR = Path(os.environ.get("KIDNEY3DCT_ROOT", Path(__file__).resolve().parents[2])) / "experiments" / "analysis" / "lesion_level_eval"
