...
PROJECT_ROOT = Path(os.environ.get("KIDNEY3DCT_ROOT", Path(__file__).resolve().parents[2]))
...
def compute_metrics_from_predictions(npz_path):
    ...
    # ---- L2: side-level per-type AUC (includes normal sides) ----
    # Also computes bilateral AUC (L+R concatenated) to boost statistical power
    # for the low-prevalence solid class.
    ...
    # (solid AUC now computed directly in the ["cyst", "solid"] loop above,
    # since solid is a first-class label.)
